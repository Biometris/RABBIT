...
function parse_commandline()
    s = ArgParseSettings()
    s.description = "Genotype imputation in connected multiparental populations"
    workdir = pwd()
    ...
    @add_arg_table! s begin
        ...
        default = "nothing"
        "--iscorrectfounder"
        help = "if true, perform parental error correction. If it is nothing, iscorrectfounder=true if model=depmodel or offspring do not have genotypes in AD format"
        arg_type = AbstractString
        default = "nothing"
        "--phasealg"
        help = phasealg_help

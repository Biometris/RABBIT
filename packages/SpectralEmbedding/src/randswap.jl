...
"""
    RandswapResult{C,D<:Real,WC<:Real} 

The output of [`randswap`](@ref) 
...
const _randswap_default_maxiter = 100
const _randswap_default_maxstuck = 3
...
function randswap(X::AbstractMatrix{<:Real},                # in: data matrix (d x n) columns = obs
                ...
                updatepair::Bool=false,                           # in: if true, update two cluster propotypes instead of one
    ...
    acceptedswap = DataFrame(iteration=[0],delcluster=[-1], newcluster=[-1],objvalue=[objv])
    ...
    while t < maxiter
        t += 1        
        acceptedswap2 = randswap_update!(X, weights, centers, assignments, dmat, costs, counts, wcounts, 
            to_update, unused, randswap_state, distance,init,updatepair,rng)
        ...
        randswap_state .= deepcopy([centers, assignments, dmat, costs, wcounts])  
        newobjv = weights === nothing ? sum(costs) : dot(weights, costs)    
        objv_change = newobjv - objv   
        objv = newobjv
        if objv_change > tol 
...
function randswap_update!(X::AbstractMatrix{<:Real},          # in: data matrix (d x n)
    ...
    updatepair::Bool,                         # in: if true, update two cluster propotypes instead of one
    rng::AbstractRNG) where T<:Real           # in: RNG object

    objv = weights === nothing ? sum(costs) : dot(weights, costs)    
    d, k = size(centers)    
    acceptedswap = DataFrame(iteration=Int[],delcluster=Int[], newcluster=Int[],objvalue=Float64[])
    for oldc in 1:k 
        for newc in 1:k 
            updatepair && oldc == newc && continue
            if updatepair
                b = [in(i,[oldc,newc]) for i in assignments]
                X2 = view(X,:,b)
                centers2 = view(centers, :,[oldc,newc])
                iseeds = Clustering.initseeds(init, X2, 2, rng=rng)
                Clustering.copyseeds!(centers2, X2, iseeds)                
                fill!(to_update, false)
                to_update[[oldc,newc]] .= true
            else
                # proposal for deleting a cluster and creating a new cluster
                newc_jls = findall(assignments .== newc)
                # newc_j = wsample(rng, newc_jls, costs[newc_jls])        
            ...
            # To minimize objv by accept/reject        
